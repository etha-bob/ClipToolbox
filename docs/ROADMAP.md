...
**Now / Next:** T1 is done. Suggested next: Q-batch (Q1–Q4) as one small PR, then B3, then decide
B1 vs. staying incremental.
...
|----|------|-------|--------|
| Q1 | Filename in window title; reset on menu; render orphaned `file_label_var` | F4 | todo |
| Q2 | One accurate error for unreadable files, return to landing (no dialog cascade) | F3 | todo |
| Q3 | Settings: Esc cancels, DONE saves (split `close()`) | F9 | todo |
| Q4 | Session-restore toast with RESET action; don't count inert crop keyframes | F6, F14 | todo |
